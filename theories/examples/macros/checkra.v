...
Section stack_macros.
  ...
  Definition checkra_pre r a p : iProp Σ :=
    ([∗ list] a_i;w_i ∈ a;(checkra_instrs_pre r off_to_blockU), a_i ↦ₐ[p] w_i)%I.
  Definition checkra r a p : iProp Σ :=
    ([∗ list] a_i;w_i ∈ a;(checkra_instrs r), a_i ↦ₐ[p] w_i)%I.

  Lemma branchperm_pre_spec r a w pc_p pc_p' pc_g pc_b pc_e a_first a_last φ w1 w2 w3 w4 :
    isCorrectPC_range pc_p pc_g pc_b pc_e a_first a_last ->
    PermFlows pc_p pc_p' ->
    contiguous_between a a_first a_last ->

      ▷ checkra_pre r a pc_p'
    ...
    ∗ ▷ (if is_cap w then
           ...
           if readAllowed p then
             (PC ↦ᵣ inr (pc_p,pc_g,pc_b,pc_e,a_last) ∗ checkra_pre r a pc_p' ∗
    ...
    ⊢
      WP Seq (Instr Executable) {{ φ }}.
  Proof.
    iIntros (Hvpc Hfl Hcont) "(>Hprog & >HPC & >Hr & >Hr_t1 & >Hr_t2 & >Hr_t3 & >Hr_t4 & Hcont)".
    ...
    iPrologue "Hprog".
    iApply (wp_lea_success_z with "[$Hi $HPC $Hr_t4]");
      [apply decode_encode_instrW_inv|auto|iCorrectPC a_first a_last|iContiguous_next_a Hcont|apply Hleaend|auto..].
    { destruct Hperms as [-> | [-> | ->] ]; auto. }
    iEpilogue "(HPC & Hi & Hr_t4)";iCombine "Hi" "Hprog_done" as "Hprog_done".
    (* jnz r_t4 r_t2 *)
    iPrologue "Hprog". iSimpl in "Hr_t2".
  ...
  Lemma checkra_spec r a w pc_p pc_p' pc_g pc_b pc_e a_first a_last φ w1 w2 w3 w4 :
    isCorrectPC_range pc_p pc_g pc_b pc_e a_first a_last ->
    PermFlows pc_p pc_p' ->
    contiguous_between a a_first a_last ->
    ...
    ∗ ▷ (if is_cap w then
           ∃ l p b e a', ⌜w = inr (p,l,b,e,a')⌝ ∧
           if readAllowed p then
             (PC ↦ᵣ inr (pc_p,pc_g,pc_b,pc_e,a_last) ∗ checkra r a pc_p' ∗
  ...
  Proof.
    ...
    iApply (branchperm_pre_spec with "[- $HPC $Hcode_first $Hr $Hr_t1 $Hr_t2 $Hr_t3 $Hr_t4]"); [apply Hvpc_code|apply Hfl|apply Hcont_code|].
    ...
    - iDestruct "Hcont" as (l' p' b e a' Heq) "Hcont". simplify_eq. rewrite Hra.
      ...
      iApply (wp_move_success_z with "[$HPC $Hi $Hr_t1]");
        [apply decode_encode_instrW_inv|eauto|iCorrectPC link a_last|iContiguous_next_a Hcont|].
      ...
      iApply (wp_move_success_z with "[$HPC $Hi $Hr_t4]");
        [apply decode_encode_instrW_inv|eauto|iCorrectPC link a_last|..].
