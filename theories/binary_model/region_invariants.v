...
| Permanent
| Revoked
| Monostatic of gmap Addr (Word * Word)
| Uninitialized of (Word * Word).
...
Class heapPreG Σ := HeapPreG {
  ...
  heapPreG_saved_pred :> savedPredG Σ (((STS_std_states Addr region_type) * (STS_states * STS_rels)) * (Word * Word));
  heapPreG_rel :> inG Σ (authR relUR);
}.

Class heapG Σ := HeapG {
  heapG_invG : invG Σ;
  heapG_saved_pred :> savedPredG Σ (((STS_std_states Addr region_type) * (STS_states * STS_rels)) * (Word * Word));
...
Instance subG_heapPreΣ {Σ}:
  subG heapPreΣ Σ →
  invPreG Σ →
  subG (savedPredΣ (((STS_std_states Addr region_type) * (STS_states * STS_rels)) * (Word * Word))) Σ →
  heapPreG Σ.
Proof. solve_inG. Qed.
...
Section REL_defs.
  ...
  Definition rel_def (l : Addr) (φ : ((STS_std_states Addr region_type * (STS_states * STS_rels)) * (Word * Word)) -> iProp Σ) : iProp Σ :=
...
Section heap.
  Context {Σ:gFunctors} {memg:memG Σ} {regg:regG Σ}
          {stsg : STSG Addr region_type Σ} {heapg : heapG Σ} {cfgg : cfgSG Σ}
          `{MachineParameters}.
  ...
  Global Instance rel_persistent l (φ : (WORLD * (Word * Word)) -> iProp Σ) :
    Persistent (rel l φ).
  Proof. rewrite rel_eq /rel_def REL_eq /REL_def. apply _. Qed.

  Definition future_pub_mono (φ : (WORLD * (Word * Word)) -> iProp Σ) (v1 v2 : Word) : iProp Σ :=
    (□ ∀ W W', ⌜related_sts_pub_world W W'⌝ → φ (W,(v1,v2)) -∗ φ (W',(v1,v2)))%I.

  Definition future_pub_plus_mono (φ : (WORLD * (Word * Word)) -> iProp Σ) (v1 v2  : Word) : iProp Σ :=
    (□ ∀ W W', ⌜related_sts_pub_plus_world W W'⌝ → φ (W,(v1,v2)) -∗ φ (W',(v1,v2)))%I.

  Definition future_pub_a_mono (a : Addr) (φ : (WORLD * (Word * Word)) -> iProp Σ) (v1 v2 : Word) : iProp Σ :=
    (□ ∀ W W', ⌜related_sts_a_world W W' a⌝ → φ (W,(v1,v2)) -∗ φ (W',(v1,v2)))%I.

  Definition future_priv_mono (φ : (WORLD * (Word * Word)) -> iProp Σ) v1 v2 : iProp Σ :=
    (□ ∀ W W', ⌜related_sts_priv_world W W'⌝ → φ (W,(v1,v2)) -∗ φ (W',(v1,v2)))%I.
  ...
  Definition monostatic (W : WORLD) (m: gmap Addr (Word * Word)) (l : Addr) :=
    match W.1 !! l with
    | Some ρ => ρ = (Monostatic m)
    | _ => False
    end.
  Definition uninitialized (W : WORLD) (w1 w2:Word) (l : Addr) :=
    match W.1 !! l with
    | Some ρ => ρ = Uninitialized (w1,w2)
    | _ => False
    end.
  ...
  Definition region_map_def M (Mρ: gmap Addr region_type) W :=
    ([∗ map] a↦γpred ∈ M, ∃ ρ, ⌜Mρ !! a = Some ρ⌝ ∗
                            ...
                            match ρ with
                            | Monotemporary => ∃ (v1 v2 : Word),
                                               a ↦ₐ v1 ∗ a ↣ₐ v2
                                             ∗ future_pub_a_mono a φ v1 v2
                                             ∗ ▷ φ (W,(v1,v2))
                            | Permanent => ∃ (v1 v2 : Word),
                                               a ↦ₐ v1 ∗ a ↣ₐ v2
                                             ∗ future_priv_mono φ v1 v2
                                             ∗ ▷ φ (W,(v1,v2))
                            | Monostatic m => ∃ v1 v2, ⌜m !! a = Some (v1,v2)⌝
                                             ∗ a ↦ₐ v1 ∗ a ↣ₐ v2
                                             ∗ ⌜∀ a', a' ∈ dom (gset Addr) m →
                                                      Mρ !! a' = Some (Monostatic m)⌝
                            | Uninitialized (w1,w2) => a ↦ₐ w1 ∗ a ↣ₐ w2
                            | Revoked => emp
                            end)%I.
  ...
  Proof.
    ...
    iDestruct "Hl" as (v v') "(#Hmono & Hφ & Hφ' & Hothers)".
    iExists _. do 2 (iSplitR;[eauto|]).
  ...
  Proof.
    ...
    iFrame. destruct ρ; try by iFrame.
    iDestruct "HH" as (φ Hpers) "(#Hsavedφ & Hl)".
    iDestruct "Hl" as (v v') "(% & Hφ & Hφ' & Hothers)".
    iExists _. do 2 (iSplitR;[eauto|]).
    iDestruct "Hothers" as %Hothers. iFrame "#".
    iExists _,_. iSplitR; eauto. iFrame. iPureIntro.
    intros a' Ha'. destruct (decide (a' = l)).
  ...
  Proof.
    ...
    iDestruct "Hl" as (v v') "(Hl & Hl' & #Hmono & Hφv)".
    iDestruct (saved_pred_agree _ _ _ (W,(v,v')) with "Hφ Hφ'") as "#Hφeq".
    iExists v,v'. iFrame.
    iSplitR "Hφv".
    ...
    - iSplitR.
      + rewrite /future_pub_mono.
        ...
        iDestruct (saved_pred_agree _ _ _ (a,(v,v')) with "Hφ Hφ'") as "#Hφeq'".
        iDestruct (saved_pred_agree _ _ _ (a0,(v,v')) with "Hφ Hφ'") as "#Hφeq''".
  ...
  Lemma region_open_uninitialized W l v v' φ :
    (std W) !! l = Some (Uninitialized (v,v')) →
    rel l φ ∗ region W ∗ sts_full_world W -∗
        open_region l W
        ∗ sts_full_world W
        ∗ sts_state_std l (Uninitialized (v,v'))
        ∗ l ↦ₐ v
        ∗ l ↣ₐ v'.
  Proof.
    iIntros (Htemp) "(Hrel & Hreg & Hfull)".
    ...
    rel l φ ∗ region W ∗ sts_full_world W -∗
        ∃ v v', open_region l W
           ...
           ∗ (▷ if (decide (ρ = Monotemporary))
                then future_pub_a_mono l φ v v'
                else future_priv_mono φ v v')
           ∗ ▷ φ (W,(v,v')).
  Proof.
    iIntros (Hne Htemp) "(Hrel & Hreg & Hfull)".
    destruct ρ; destruct Hne as [Hne | Hne]; try (exfalso; congruence).
    - iDestruct (region_open_monotemp with "[$Hrel $Hreg $Hfull]") as (v v') "(Hr & Hfull & Hstate & Hl & Hl' & Hmono & φ)"; auto.
      iExists _,_; iFrame.
    - iDestruct (region_open_perm with "[$Hrel $Hreg $Hfull]") as (v v') "(Hr & Hfull & Hstate & Hl & Hl' & Hmono & φ)"; auto.
      iExists _,_; iFrame.
  Qed.
  ...
  Proof.
    ...
    iDestruct "Hl" as (v v' HH2) "(Hl & Hl' & Hothers)".
  ...
  Proof.
    ...
    iDestruct "Hl" as (v v' Hpv') "[Hl [Hl' #Hall]]". iDestruct "Hall" as %Hall.
    iDestruct (big_sepM_delete _ _ l with "[$Hr Hl Hl' Hstate]") as "Hr";[eauto|..].
    { iExists ρ. iSplitR;subst;auto. iFrame. iExists _. repeat iSplit;eauto. iExists _,_. iFrame. auto. }
  ...
  Lemma region_close_monotemp W l φ v v' `{forall Wv, Persistent (φ Wv)} :
    ⊢ sts_state_std l Monotemporary
      ∗ open_region l W ∗ l ↦ₐ v ∗ l ↣ₐ v' ∗ future_pub_a_mono l φ v v' ∗ ▷ φ (W,(v,v')) ∗ rel l φ
      -∗ region W.
  Proof.
    rewrite open_region_eq rel_eq region_eq /open_region_def /rel_def /region_def
            REL_eq RELS_eq /RELS_def /REL_def.
    iIntros "(Hstate & Hreg_open & Hl & Hl' & #Hmono & Hφ & #Hrel)".
  ...
  Lemma region_close_mono_uninit_w E W l φ w v v' `{forall Wv, Persistent (φ Wv)} :
    sts_state_std l (Uninitialized w)
    ∗ open_region l W
    ∗ l ↦ₐ v
    ∗ l ↣ₐ v'
    ∗ future_pub_a_mono l φ v v'
    ∗ ▷ φ (W,(v,v')) (* Maybe φ (<l[ l := Temporary, (Rpub, Rpriv) ]l> W, v) *)
    ∗ rel l φ
    ∗ sts_full_world W
  ...
  Proof.
    rewrite open_region_eq rel_eq region_eq /open_region_def /rel_def /region_def
            REL_eq RELS_eq /RELS_def /REL_def.
    iIntros "(Hstate & Hreg_open & Hl & Hl' & #Hmono & #Hφ & #Hrel & Hfull)".
    iDestruct "Hrel" as (γpred) "#[Hγpred Hφ_saved]".
    ...
    { iFrame. iExists Monotemporary. iFrame.
      ...
      iExists _. iFrame "∗ #". repeat iSplitR; auto. iExists _,_. iFrame. auto. }
  ...
  Lemma region_close_perm W l φ v v' `{forall Wv, Persistent (φ Wv)}:
    ⊢ sts_state_std l Permanent
      ∗ open_region l W ∗ l ↦ₐ v ∗ l ↣ₐ v' ∗ future_priv_mono φ v v' ∗ ▷ φ (W,(v,v')) ∗ rel l φ
      -∗ region W.
  Proof.
    rewrite open_region_eq rel_eq region_eq /open_region_def /rel_def /region_def
            REL_eq RELS_eq /RELS_def /REL_def.
  ...
  Lemma region_close W l φ v v' (ρ : region_type) `{forall Wv, Persistent (φ Wv)} :
    ρ = Permanent ∨ ρ = Monotemporary→
    sts_state_std l ρ
                  ∗ open_region l W ∗ l ↦ₐ v ∗ l ↣ₐ v' ∗
                  (if (decide (ρ = Monotemporary))
                   then future_pub_a_mono l φ v v'
                   else future_priv_mono φ v v') ∗ ▷ φ (W,(v,v')) ∗ rel l φ
    -∗ region W.
  Proof.
  ...
  Lemma region_open_next_monouninit W w w' ls l φ :
    l ∉ ls →
    (std W) !! l = Some (Monostatic {[l:=(w,w')]}) →
    rel l φ ∗ open_region_many ls W ∗ sts_full_world W -∗
        open_region_many (l :: ls) W
        ∗ sts_full_world W
        ∗ sts_state_std l (Monostatic {[l:=(w,w')]})
        ∗ l ↦ₐ w ∗ l ↣ₐ w'.
  Proof.
    ...
    iDestruct "Hl" as (v v' Hlookup) "[Hl [Hl' _] ]".
  ...
  Lemma region_open_next_monouninit_w W w w' ls l φ :
    l ∉ ls →
    (std W) !! l = Some (Uninitialized (w,w')) →
    rel l φ ∗ open_region_many ls W ∗ sts_full_world W -∗
        open_region_many (l :: ls) W
        ∗ sts_full_world W
        ∗ sts_state_std l (Uninitialized (w,w'))
        ∗ l ↦ₐ w ∗ l ↣ₐ w'.
  Proof.
    iIntros (Hnin Htemp) "(Hrel & Hreg & Hfull)".
    rewrite open_region_many_eq /open_region_many_def /= /region_map_def rel_eq /rel_def REL_eq /REL_def.
    ...
    1,2,3,4,5: rewrite Htemp in Hcontr; try by inversion Hcontr.
    iDestruct "Hl" as (φ' Hpers) "[#Hsaved Hl]".
    destruct p. inversion Hcontr; subst w w'.
    iFrame.
    iExists M,Mρ. iFrame "∗ #".
  ...
  Lemma region_close_next_monotemp W φ ls l v v' `{forall Wv, Persistent (φ Wv)} :
    l ∉ ls ->
    ⊢ sts_state_std l Monotemporary ∗
      open_region_many (l::ls) W ∗ l ↦ₐ v ∗ l ↣ₐ v' ∗ future_pub_a_mono l φ v v' ∗ ▷ φ (W,(v,v')) ∗ rel l φ
      -∗ open_region_many ls W.
  Proof.
    rewrite open_region_many_eq /open_region_many_def.
    iIntros (Hnin) "(Hstate & Hreg_open & Hl & Hl' & #Hmono & Hφ & #Hrel)".
  ...
  Lemma region_close_next_mono_uninit_w E W ls l φ w v v' `{forall Wv, Persistent (φ Wv)} :
    l ∉ ls ->
    sts_state_std l (Uninitialized w)
    ∗ open_region_many (l::ls) W
    ∗ l ↦ₐ v
  ...
  Proof.
    rewrite open_region_many_eq rel_eq /open_region_many_def /rel_def /region_def
            REL_eq RELS_eq /RELS_def /REL_def.
    iIntros (Hnin) "(Hstate & Hreg_open & Hl & Hl' & #Hmono & Hφ & #Hrel & Hfull)".
    iDestruct "Hrel" as (γpred) "#[Hγpred Hφ_saved]".
    iDestruct "Hreg_open" as (M Mρ) "(HM & HMW & HMρ & Hpreds)".
    iDestruct "HMW" as %HMW. iDestruct "HMρ" as %HMρ.
    iDestruct (sts_full_state_std with "Hfull Hstate") as "%".
  ...
  Lemma region_close_next_perm W φ ls l v v' `{forall Wv, Persistent (φ Wv)} :
    l ∉ ls ->
    ⊢ sts_state_std l Permanent ∗
      open_region_many (l::ls) W ∗ l ↦ₐ v ∗ l ↣ₐ v' ∗ future_priv_mono φ v v' ∗ ▷ φ (W,(v,v')) ∗ rel l φ
      -∗ open_region_many ls W.
  Proof.
    rewrite open_region_many_eq /open_region_many_def.
    iIntros (Hnin) "(Hstate & Hreg_open & Hl & Hl' & #Hmono & Hφ & #Hrel)".
  ...
  Definition monotonicity_guarantees_region ρ l w w' φ :=
    (match ρ with
     | Monotemporary => future_pub_a_mono l
     | Permanent => future_priv_mono
     | Revoked | Monostatic _ | Uninitialized _ => λ (_ : WORLD * (Word * Word) → iProp Σ) (_ _: Word), True
     end φ w w')%I.

  Definition monotonicity_guarantees_decide ρ l w1 w2 φ:=
    (if decide (ρ = Monotemporary)
     then future_pub_a_mono l φ w1 w2
     else future_priv_mono φ w1 w2)%I.

   Lemma region_open_next
        (W : WORLD)
        (φ : WORLD * (Word * Word) → iProp Σ)
     ...
     ⊢ open_region_many ls W ∗ rel l φ ∗ sts_full_world W
       -∗ ∃ v v' : Word,
         sts_full_world W
                        ∗ sts_state_std l ρ
                        ∗ open_region_many (l :: ls) W
                        ∗ l ↦ₐ v ∗ l ↣ₐ v' ∗ ▷ monotonicity_guarantees_region ρ l v v' φ ∗
                        ▷ φ (W, (v,v')).
   Proof.
    unfold monotonicity_guarantees_region.
    intros. iIntros "H".
    destruct ρ; try congruence.
    - iDestruct (region_open_next_monotemp with "H") as (v v') "[A [B [C [D [E [F G]]]]]]"; eauto.
      iExists v,v'. iFrame.
  ...
  Lemma region_close_next
        ...
        (ls : list Addr) (l : Addr) (v v' : Word) (ρ : region_type)
        (Hρnotrevoked : ρ <> Revoked)
        (Hρnotmonostatic : ¬ exists g, ρ = Monostatic g)
        (Hρnotuninitialized : ¬ exists w, ρ = Uninitialized w):
    l ∉ ls
    → sts_state_std l ρ
                    ∗ open_region_many (l :: ls) W
                    ∗ l ↦ₐ v ∗ l ↣ₐ v' ∗ monotonicity_guarantees_region ρ l v v' φ ∗ ▷ φ (W, (v,v')) ∗ rel l φ -∗
                    open_region_many ls W.
  Proof.
    unfold monotonicity_guarantees_region.
